		...
		if !file_data.nil?
			CSV.foreach(file_data.path) do |row|
		  	...
		  	@student = Student.new(@student_params)
				...
				User.update_all("heir_id = '#{@predecessor_id}'", "heir_id = '#{@student_id}'")
  ...
  private
    ...
    def set_student
      @student = Student.find(params[:id])
		...
		def access
			...
			unless @usr.admin? or @student.id == session[:user_id]
    ...
    def student_params
      	...
      	params.require(:student).permit(:user_name, :name, :password, :grade, :number, :diploma_work_id, :access)
				...
				params.require(:student).permit(:user_name, :name, :password, :grade, :number, :diploma_work_id)
			end
    end
end
